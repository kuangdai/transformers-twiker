...
class TwikerModel(nn.Module):
    def __init__(self, vocab_size: int, kernel_size: int, n_head: int, n_layer: int,
                 ...
                 causal_handling: str = "shrink_near_boundary", ):
        super().__init__()
        ...
        ], f"Unknown value for `causal_handling`: {causal_handling}."
        ...
        # prepare causal mask
        if causal_handling == "none":
            self.causal_mask = None
        elif causal_handling == "only_left_half":
            self.causal_mask = torch.ones(self.kernel_size)
            self.causal_mask[p + 1:].zero_()
        else:
            self.causal_mask = torch.ones(p + 1, self.kernel_size)
            for i_mask in range(1, p + 1):
                self.causal_mask[i_mask, -i_mask:].zero_()  # 11111 => 11110
                if causal_handling == "shrink_near_boundary":
                    self.causal_mask[i_mask, :i_mask].zero_()  # 11110 => 01110
    ...
    def conv_key_value(self, key: torch.Tensor, value: torch.Tensor, kernel: torch.Tensor,
        ...
        # causal handling
        if for_causal:
            if self.causal_handling == "only_left_half":
                kernel = kernel * self.causal_mask.to(
                    dtype=kernel.dtype, device=kernel.device)[None, None, None, :]
            elif self.causal_handling in ["truncate_near_boundary", "shrink_near_boundary"]:
                ...
                pass  # self.causal_handling == "none"
        ...
        # rearrange output
        if for_causal and self.causal_handling in ["truncate_near_boundary", "shrink_near_boundary"]:
            kv = kv.reshape(n_batch, 2 * n_head, p + 1, n_token, n_feat).movedim(2, -1)
            key, value = kv[..., 0].split(n_head, dim=1)
            causal_boundary_keys, causal_boundary_values = kv[..., 1:].split(n_head, dim=1)
            return key, value, causal_boundary_keys, causal_boundary_values
        else:
            key, value = kv.split(n_head, dim=1)
            return key, value, None, None

    @staticmethod
    def correct_attn_weights_near_causal_boundary(attn_weights: torch.Tensor,
                                                  query: torch.Tensor,
                                                  causal_boundary_keys: torch.Tensor):
        n_past = attn_weights.size(-1) - attn_weights.size(-2)
        p = causal_boundary_keys.size(-1)
        for i_mask in range(0, p):
            offset = p - i_mask - 1
            cb_key = causal_boundary_keys[..., i_mask]  # 01110, 00100
    ...
    @staticmethod
    def correct_attn_output_near_causal_boundary(attn_output: torch.Tensor,
                                                 attn_weights: torch.Tensor,
                                                 value: torch.Tensor,
                                                 causal_boundary_values: torch.Tensor):
        n_past = attn_weights.size(-1) - attn_weights.size(-2)
        p = causal_boundary_values.size(-1)
        for i_mask in range(0, p):
            offset = p - i_mask - 1
            cb_value = causal_boundary_values[..., i_mask]  # 01110, 00100
